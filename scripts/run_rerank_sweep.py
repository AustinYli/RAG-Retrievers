...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Sweep rerank_top_k over the best base per dataset."
    )
    ...
    parser.add_argument(
        "--device",
        help="Torch device. Omit to use Sentence Transformers auto-selection.",
    )
    parser.add_argument("--datasets", help="Comma-separated dataset filter.")
    parser.add_argument(
        "--base-run-names", help="Comma-separated base run-name filter."
    )
    parser.add_argument(
        "--rerank-top-k-values",
        help="Comma-separated rerank depths. Defaults to 10,20,50,100,200.",
    )
    parser.add_argument("--trust-remote-code", action="store_true")
    parser.add_argument(
        "--force",
        action="store_true",
        help="Replace the output CSV before writing rows.",
    )
    ...
    if args.force and output_path.exists():
        output_path.unlink()
    annotate_latency_method(output_path)
    rerank_top_k_values = parse_int_csv(args.rerank_top_k_values) or RERANK_TOP_K_VALUES
    existing = existing_run_names(output_path)
    selected_bases = selected_bases_by_dataset(
        rows,
        datasets=parse_str_csv(args.datasets),
        base_run_names=parse_str_csv(args.base_run_names),
        limit=2,
    )
    pending: dict[str, list[tuple[dict[str, str], list[int]]]] = {}
    for dataset, base_rows in selected_bases.items():
        for base_row in base_rows:
            missing_depths = [
                depth
                for depth in rerank_top_k_values
                if make_run_name(
                    dataset, args.reranker_model, base_row["run_name"], depth
                )
                not in existing
            ]
            if missing_depths:
                pending.setdefault(dataset, []).append((base_row, missing_depths))
    if not pending:
        print("All selected rerank runs already exist.")
        return
    ...
    for dataset, base_rows in pending.items():
        corpus, queries, qrels = load_beir_dataset(
            dataset=dataset, split="test", data_dir="data/beir"
        )
        for base_row, missing_depths in base_rows:
            ...
            runs_by_depth, latencies_by_depth = rerank_at_depths_with_latencies(
                ...
                base_runs=base_run,
                model=model,
                rerank_top_k_values=missing_depths,
                keep_top_k=1000,
                batch_size=args.batch_size,
            )
            sweep_wall_seconds = time.perf_counter() - sweep_started
            for rerank_top_k in missing_depths:
                run = runs_by_depth[rerank_top_k]
                rerank_latencies = latencies_by_depth[rerank_top_k]
                search_seconds = sum(rerank_latencies)
                ...
                per_query_path = write_per_query_metrics(
                    per_query_dir, run_name, per_query
                )
                run_artifact_path = write_run_artifact(per_query_dir, run_name, run)
                row: dict[str, Any] = {
                    ...
                    "latency_method": (
                        "single_depth"
                        if len(missing_depths) == 1
                        else "incremental_cumulative"
                    ),
                    "query_latency_method": "component_percentile_sum",
                    "model_load_seconds": round(model_load_seconds, 3),
                    "sweep_wall_seconds": round(sweep_wall_seconds, 3),
                    "search_seconds": round(search_seconds, 3),
                    "throughput_ms_per_query": round(
                        search_seconds / len(queries) * 1000.0, 3
                    ),
                    "rerank_latency_p50_ms": round(
                        percentile(rerank_latencies, 50) * 1000.0, 3
                    ),
                    "rerank_latency_p95_ms": round(
                        percentile(rerank_latencies, 95) * 1000.0, 3
                    ),
                    "query_latency_p50_ms": round(
                        base_latency_p50 + percentile(rerank_latencies, 50) * 1000.0, 3
                    ),
                    "query_latency_p95_ms": round(
                        base_latency_p95 + percentile(rerank_latencies, 95) * 1000.0, 3
                    ),
...
def best_bases_by_dataset(
    rows: dict[str, dict[str, str]], limit: int
    ...
    return {
        dataset: sorted(
            dataset_rows, key=lambda row: float(row["ndcg@10"]), reverse=True
        )[:limit]
...
def selected_bases_by_dataset(
    ...
    return {
        dataset: base_rows
        for dataset, base_rows in selected.items()
...
def existing_run_names(path: Path) -> set[str]:
    if not path.exists():
        return set()
    with path.open("r", newline="", encoding="utf-8") as file:
...
def annotate_latency_method(path: Path) -> None:
    if not path.exists():
        return
    with path.open("r", newline="", encoding="utf-8") as file:
        ...
        rows = list(reader)
    if not rows or (
        "query_latency_method" in fieldnames
        and all(row.get("query_latency_method") for row in rows)
    ):
        return
    if "query_latency_method" not in fieldnames:
        fieldnames.append("query_latency_method")
    for row in rows:
        row["query_latency_method"] = "component_percentile_sum"
...
def model_slug(model_name: str) -> str:
    return "".join(
        char if char.isalnum() else "-" for char in model_name.lower()
    ).strip("-")


def make_run_name(
    dataset: str, reranker_model: str, base_run_name: str, depth: int
) -> str:
    return (
        f"{dataset}_rerank_sweep_{model_slug(reranker_model)}_"
        f"{base_run_name}_top{depth}"
    )
...
def write_run_artifact(
    ...
    output_dir.mkdir(parents=True, exist_ok=True)
    path = output_dir / f"{run_name}.run.jsonl"
